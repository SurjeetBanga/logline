...
const { LogsController, openSource } = withVscode(mock, () => ({
  ...require('./vscode/logs-controller') as typeof import('./vscode/logs-controller'),
  ...require('./vscode/source-navigation') as typeof import('./vscode/source-navigation')
}));
...
test('ambiguous Java frames prompt for a workspace file; missing files stay unopened', async () => {
  ...
  await openSource(provider.store, { id: 2, block: 0, line: 0 });
