...
const { slugify, nextServerId, resolveAutoStartServers, resolveRunTarget, resolveCwd } = require('./server-config');
...
test('resolveAutoStartServers only runs autoStart servers, and reports untrusted workspaces', () => {
  const servers = [{ id: 'a', autoStart: true }, { id: 'b', autoStart: false }];
  ...
  assert.deepEqual(resolveAutoStartServers([{ id: 'b', autoStart: false }], false), { blocked: false, servers: [] });
});

test('resolveRunTarget looks up a saved server, forwards its env, and falls back to the workspace cwd', () => {
  const servers = [
    { id: 'api', command: 'npm start', env: { PORT: '3000' } },
    { id: 'worker', command: 'npm run worker', cwd: '/srv/worker' }
  ];
...
test('a saved server cwd expands ${workspaceFolder} like tasks.json does', () => {
  const servers = [{ id: 'api', label: 'API', command: 'npm run dev', cwd: '${workspaceFolder}/node' }];
